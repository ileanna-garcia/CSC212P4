...
public class SpookyMansion implements GameWorld {
	...
	public SpookyMansion() {
		...
		basement.addExit(new Exit("entranceHall", "Let's go back up."));
		basement.addExit(new Exit("dungeon", "Let's look at our secret"));

		
		Place dungeon = insert( Place.create ("dungeon", "Why are there chains here? Oh no..This is a dungeon!" ));
		dungeon.addExit(new Exit("basement", "Let's go back to the basement this is creepy."));
		...
		attic.addExit(new Exit("attic3", "Looks like there's something here too"));
		...
		Place attic3 = insert(Place.create("attic3", "There's some light here.\n" +"Looks like it's just a broken light."));
		attic3.addExit(new Exit("attic", "Let's go back through the archway."));
		...
		secretRoom.addExit(new Exit("basement", "What's this room on the left?"));
